			       ...
			       void *p, int size)
{
	...
	if (npu->soc_data->version == NPU_V1 && func_id >= NPU_FUNC_DBA)
		return -EOPNOTSUPP;
				   ...
				   void *data, int data_len, gfp_t gfp)
{
	...
	dev_info(npu->dev, "wlan get msg: id %d, core %d, body size %d", func_id, ifindex, data_len);
...
static void airoha_boot_core_v1(struct airoha_npu *npu, struct reserved_mem *rmem) {
	u32 val, coreToStop;
	coreToStop = of_property_read_bool(npu->dev->of_node, "core_to_stop");
	if (coreToStop)
		of_property_read_u32(npu->dev->of_node, "core_to_stop", &coreToStop);
	...
	for (int core = 0; core < npu->soc_data->max_cores; core++) {
		...
		if (core == 0) {
			regmap_write(npu->regmap, REG_CR_BOOT_CONFIG(npu->soc_data), 0x1);
			...
			regmap_write(npu->regmap, REG_CR_BOOT_TRIGGER(npu->soc_data), 0x2);
		}

		msleep(100);
		if (coreToStop == core+1)
			break;
	}
}
...
static bool isFpga(struct regmap *scuclk)
{
	u32 isFPGA;
	regmap_read(scuclk, 0x9c, &isFPGA);
	return (isFPGA & 0x1) == 0;
}
static int l2CSramSize(struct regmap *scuclk)
{
	u32 l2c_sram_size;
...
static int airoha_npu_probe(struct platform_device *pdev)
{
	...
	if (npu->soc_data->version >= NPU_V2) {
		err = dma_set_coherent_mask(dev, 0xbfffffff /*DMA_BIT_MASK(32)*/);
		if (err)
	...
	if (!err)
		...
		dev_info(npu->dev, "NPU not support get version");
	else
